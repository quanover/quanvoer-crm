...
<script type="text/javascript">
			...
			$.ajax({
				...
				success:function (resp) {
					...
					if(resp.success){
						...
						$("#h5"+id).html(resp.remark.noteContent)
						$("#small"+id).html(resp.remark.editTime+"由"+resp.remark.editBy)
			...
			$.ajax({
				...
				success:function (resp) {
					...
					if(resp.success){
						...
						html+='<h5 id="h5'+resp.remark.id+'">'+resp.remark.noteContent+'</h5>';
						html+='<font color="gray">市场活动</font> <font color="gray">-</font> <b>${a.name}</b> <small style="color: gray;" id="small'+resp.remark.id+'">'+(resp.remark.createTime)+'由'+(resp.remark.createBy)+'</small>';
						html+='<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
						html+='<a class="myHref" href="javascript:void(0);" onclick="editRemark(\''+resp.remark.id+'\')"><span class="glyphicon glyphicon-edit" style="font-size: 20px; color: #FF0000;"></span></a>';
						html+='&nbsp;&nbsp;&nbsp;&nbsp;';
						html+='<a class="myHref" href="javascript:void(0);" onclick="deleteRemark(\''+resp.remark.id+'\')"><span class="glyphicon glyphicon-remove" style="font-size: 20px; color: #FF0000;"></span></a>';
						html+='</div>';
						html+='</div>';
						html+='</div>';
			...
			success:function (resp) {
				...
				$.each(resp,function (index, e) {
					/*
						...
						在遍历中，动态生成的元素直接触发事件最方便
							方法中的参数必须套用在字符串中
					 */
					html+='<div  id="'+e.id+'" class="remarkDiv" style="height: 60px;">';
					html+='<img title="zhangsan" src="image/user-thumbnail.png" style="width: 30px; height:30px;">';
					html+='<div style="position: relative; top: -40px; left: 40px;" >';
					html+='<h5 id="h5'+e.id+'">'+e.noteContent+'</h5>';
					html+='<font color="gray">市场活动</font> <font color="gray">-</font> <b>${a.name}</b> <small style="color: gray;" id="small'+e.id+'">'+(e.editFlag==0?e.createTime:e.editTime)+'由'+(e.editFlag==0?e.createBy:e.editBy)+'</small>';
					html+='<div style="position: relative; left: 500px; top: -30px; height: 30px; width: 100px; display: none;">';
	...
	function editRemark(id) {
		$("#remarkId").val(id)
		$("#noteContent").val($.trim($("#h5"+id).html()))
